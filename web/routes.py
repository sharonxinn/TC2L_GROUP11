...
def match_passenger(driver_id):
    ...
    profile = Profile.query.filter_by(user_id=current_user.id).first()
    return render_template('match_passenger.html', driver=driver, passengers=passengers, profile_dict=profile_dict, user=current_user,profile=profile)
...
def booking_history():
    ...
    return render_template('booking_history.html', matches=passenger_matches, profile_dict=profile_dict,profile=profile)
